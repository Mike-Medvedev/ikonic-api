...
client = Vonage(
    Auth(api_key=os.getenv("VONAGE_API_KEY"), api_secret=os.getenv("VONAGE_API_SECRET"))
)
...
async def profile(user_id: str):
    ...
    cursor = ikonic_db_connection.cursor()
    res = cursor.execute(
        """ SELECT user_id, firstname, lastname, phone_number FROM users WHERE user_id = ? """,
        (user_id,),
    )
    row = res.fetchone()
    ikonic_db_connection.close()
...
def get_invited_users(selectedTrip: int):
    ...
    if not selectedTrip:
        ikonic_db_connection.close()
        raise HTTPException(status_code=400, detail="Please provide a valid trip ID")
    try:
        rows = cursor.execute(
            """
        ...
        rsvp_groups = {"accepted": [], "pending": [], "uncertain": [], "declined": []}
...
async def create_trip(request: Request):
    ...
    try:
        ...
        ikonic_db_connection.close()
        raise HTTPException(
            status_code=400, detail=f"Missing field in request data: {e}"
        ) from e
...
async def get_trips(request: Request):
    ...
    if not user_id:
        ...
        raise HTTPException(status_code=401, detail="Missing Authorization Header")
    res = cursor.execute(
        """SELECT trips.* FROM trips JOIN trips_users_mapping ON trips.id
                         = trips_users_mapping.trip_id WHERE trips_users_mapping.user_id = ?""",
        (user_id,),
    )
    row = res.fetchall()
    ikonic_db_connection.close()
...
async def get_trip(selectedTripId: str):
    ...
    res = cursor.execute(
        """SELECT trips.*, users.* FROM trips JOIN users ON users.user_id = trips.owner WHERE trips.id = ?""",
        (selectedTripId,),
    )
    trip = res.fetchone()
    ikonic_db_connection.close()
...
def delete_post(trip_id: str):
    ...
    if not trip_id:
        ikonic_db_connection.close()
        raise HTTPException(status_code=400, detail="Please select a valid trip id")
...
async def invite_user(request: Request):
    ...
    try:
        ...
        cursor = ikonic_db_connection.cursor()
        cursor.execute(
            "UPDATE trips_users_mapping SET rsvp = ? WHERE trip_id = ? AND user_id = ?",
            (
                "pending",
                trip_id,
                user_id,
...
async def rsvp(request: Request):
    try:
        ...
        if not user_id or not trip_id or not user_response:
            ...
            raise HTTPException(
                status_code=400,
                detail="Please provide the correct payload when rsvping",
            )
        cursor.execute(
            """
...
def create_car(trip_id: int, car: NewCar):
    try:
        ...
        cursor = ikonic_db_connection.cursor()
        cursor.execute(
            "INSERT INTO cars (trip_id, owner, seat_count) VALUES (?, ?, ?)",
...
def add_passenger(car_id: int, user_id: str, seat_position: int):
    ...
    try:
        ...
        cursor.execute(
            ...
            ON CONFLICT(car_id, seat_position) DO UPDATE SET user_id = excluded.user_id;""",
            (
                car_id,
                user_id,
                seat_position,
...
async def update_paid_status(trip_id: str, user_id: str, request: Request):
    ...
    if "new_status" not in data:
        raise HTTPException(status_code=400, detail="Error: no new_status given")
    ...
    try:
        ...
        cursor = ikonic_db_connection.cursor()
        cursor.execute(
            "UPDATE trips_users_mapping SET paid = ? WHERE trip_id = ? AND user_id = ?",
            (int(new_status), trip_id, user_id),
        )
        ikonic_db_connection.commit()
    except Exception as e:
        print(e)
